...
def get_auth_url():
    ...
    if _spotify_service_instance is None:
        _spotify_service_instance = SpotifyService()
    return _spotify_service_instance.get_oauth_client().get_authorize_url()
...
def process_callback(code):
    ...
    try:
        ...
        logger.exception("Errore durante il processo di callback OAuth")
...
class SpotifyService:
    ...
    def get_user_data(self, sp_user):
        ...
        user.top_genres = dict(Counter(genres).most_common())
        return user
